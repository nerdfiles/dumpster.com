...
var D=new RegExp(A);var C=D.exec(window.location.href);if(C==null){return""}else{return C[1]}};(function(){FSR.siteid=FSR.getParameter("siteid");
FSR.site=FSR.sites[FSR.siteid];if(!FSR.site.name){FSR.site.name=FSR.getParameter("name")}if(!FSR.site.domain){FSR.site.domain=FSR.getParameter("domain")
}if(FSR.site.domain=="default"){FSR.site.domain=false}var C=["files","js_files","image_files","html_files","css_files"];
for(var B=0,A=C.length;B<A;B++){if(FSR.site[C[B]]){FSR[C[B]]=FSR.site[C[B]]}}FSR.$S={canceled:false}
})();FSR.qualified=function(){FSR.controller.process()};FSR.qualify=function(A){FSR.$S.canceled=false;
if(A){FSR.$S.qid=A;FSR.c().set("q",A)}};FSR.cancel=function(){FSR.$S.canceled=true};FSR.QualifierController=new FSR.Class({Implements:FSR.Options,options:{},initialize:function(A){this.setOptions(A);
FSR.controller=this},load:function(){fsr$dbug.log("Qualifier: load");if(this.running){return this
}this.running=true;var A=this;new FSR.Asset.javascript((FSR.js_files||FSR.files)+"foresee-surveydef.js",{id:"foresee-survey",onload:function(){fsr$dbug.log("Survey Definitions Loaded.");
A.run()}})},run:function(){},unload:function(){fsr$dbug.log("Qualifier: unload");if(!this.running){return this
}this.running=false},process:function(){this.configure();this.launch()},configure:function(B){var D=FSR.surveydefs;
var E=FSR.c();var A=E.get("sd");var C=D[A];C.idx=A;C.survey=FSR.$merge(FSR.$P().survey,C.survey||{});
C.cancel=FSR.$merge(FSR.$P().cancel,C.cancel||{});this.sd=C},launch:function(){if(!FSR.$S.canceled){this.popSurvey()
}else{this.popCancel()}},popSurvey:function(){var F=FSR.services.survey;var H=this.sd.survey;var C=(window.screen.width-H.width)/2;
var K=(window.screen.height-H.height)/2;var L=document.location.protocol;var M=F.host;var N=F.path;
var I=FSR.c();var B=I.get("rid")||FSR.generateid();var J=this.hash(B);var G=I.get("c")||"";var D=new FSR.Hash({sid:this.sid(),cid:FSR.id,pattern:G,a:B,b:J,c:24*60*60*1000,version:FSR.version}).toQueryString();
var E=FSR.CPPS.toQueryString();var A=L+"//"+M+N+F.url+"?"+D+"&"+E;if(H.loading){this.page(FSR.$P().loading);
A=(FSR.html_files||FSR.files)+FSR.$P().loading.url+"?url="+A}FSR.log(400,FSR.c().get("c"));this.pop(A,C,K,H.width,H.height)
},popCancel:function(){var B=this.sd.cancel;this.page(B);var A=B.url;FSR.log(500,FSR.c().get("c"));
this.pop(A)},pop:function(B,E,D,C,A){var F=arguments.length;(function(){if(F>1){window.moveTo(E,D);
window.resizeTo(C,A)}window.focus();document.location.href=B}).fsr$delay(0)},sid:function(){var G=FSR.c();
var B=this.sd.name;var C=this.sd.site;if(C){B=B+"-"+C}var E=this.sd.section;if(E){B=B+"-"+E}var A=FSR.getParameter("when");
if(A){B=B+"-"+A}var D=G.get("q");if(D){B=B+"-"+D}var F=G.get("l");if(F){B=B+"-"+F}return B},page:function(E){var B=FSR.c().get("l");
if(!B){return }var D=E.locales||[];for(var C=0,A=D.length;C<A;C++){if(D[C].locale==B){E.url=D[C].url;
break}}},hash:function(B){var A=B.split("_");return((A[0]*3)+1357)+""+((A[1]*9)+58)}});new FSR.QualifierController({});
window.fsr$addEvent("domready",function(){fsr$dbug.log("Qualifier: domready");(function(){FSR.controller.load()
