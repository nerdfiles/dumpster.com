...
return }FSR.$clear(this.timer);var B=new Date().getTime();FSR.$M.run=FSR.$M.run+(B-A)},run:function(D){var A=new Date().getTime();
if(D){FSR.c().erase("pa")}var C=false;FSR.sd=this.sd=this.pd;FSR.sdi=this.sdi=this.pdi;if(this.sd){C=this.process()
...
}if(!this.inviteAccepted()){this.setupLinks("attach",this.popAttach)}if(this.trackerRunning()){this.setupLinks("pause",this.pause)
...
}});this.dhtml_css=1;new FSR.Asset.css((FSR.css_files||FSR.files)+FSR.$P().invite.css);if(FSR.$P().qualifier.css&&(FSR.$P().qualifier.css!=FSR.$P().invite.css)){new FSR.Asset.css((FSR.css_files||FSR.files)+FSR.$P().qualifier.css)
