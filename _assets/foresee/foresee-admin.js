...
};(function(){function G(K){if(typeof K=="object"){var J=K.constructor.toString().match(/array/i);
...
}}if(!FSR.site.name){FSR.site.name=B[0]}var A=["files","js_files","image_files","html_files","css_files"];
for(var F=0,H=A.length;F<H;F++){if(FSR.site[A[F]]){FSR[A[F]]=FSR.site[A[F]]}}break}}if(B){break}}})();
FSR.AdminController=new FSR.Class({Implements:FSR.Options,options:{},initialize:function(A){this.setOptions(A);
...
A.run()}})},run:function(){var N=new FSR.Hash.Cookie("fsr.sp",{path:"/",domain:FSR.site.domain||false});
...
}var Q=P[M].criteria;if(FSR.$defined(Q.sp)){var K;var R;this.row(A,P[M].name,P[M].section||R,K,N,Q.sp,Q.lf)
}var H=Q.locales||[];for(var L=0,I=H.length;L<I;L++){var R;this.row(A,P[M].name,P[M].section||R,H[L].locale,N,H[L].sp,H[L].lf)
...
F.fsr$adopt(J)},set:function(){var A=new FSR.Hash.Cookie("fsr.sp",{path:"/",domain:FSR.site.domain||false,duration:1});
FSR.Array.each($$fsr("input"),function(B){if(B.type=="text"){if(B.value!=""){A.set(B.name,B.value)
}else{A.erase(B.name)}}})},clear:function(){var A=new FSR.Hash.Cookie("fsr.sp",{path:"/",domain:FSR.site.domain||false});
